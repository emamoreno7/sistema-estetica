...
const SELECT_COLS =
  'id, cliente_id, nombre_firma, dni, declara_salud, acepta_tratamiento, acepta_datos, contraindicaciones, version, firmado_at, firmado_por_admin';
...
export async function guardarConsentimientoCliente(input: {
  ...
  if (error) {
    ...
    if (flat.includes('does not exist') || flat.includes('schema cache')) {
      return { consentimiento: null, error: 'Falta la migración 018 (consentimientos) en Supabase.' };
    }
