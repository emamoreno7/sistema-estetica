...
function waHrefComplement(serviceName: string, reason: string): string {
  const text = `Hola ${brand.shortName}, me interesaria conocer mas sobre "${serviceName}" como complemento a mi turno. Contexto profesional que vi en el portal: ${reason}`;
...
export function PortalCitasTab(props: {
  ...
  const { firmado: consentFirmado, noMigrado: consentNoMigrado, setConsentimiento } = useConsentimiento();

  const [bookingOpen, setBookingOpen] = useState(false);
  const [consentOpen, setConsentOpen] = useState(false);
  const [successSaved, setSuccessSaved] = useState<CitaClienteRow | null>(null);

  const showDbHero = proximaCita && parseCitaMomentLocal(proximaCita).getTime() >= Date.now() - 60_000;
  const planHero = activeTreatment && activeTreatment.fechaPlanPendiente === false ? activeTreatment : null;
  ...
  return (
    <div className="space-y-6">
      <motion.section
        ...
        style={{ borderColor: 'rgba(242,215,213,0.55)', background: 'var(--bg-cream)', boxShadow: PORTAL_PANEL_SHADOW }}
      >
        <p className="text-serif-premium text-lg leading-relaxed sm:text-xl" style={{ color: 'var(--primary-navy)' }}>
          ...
          <span className="font-normal" style={{ color: 'var(--text-muted)' }}>agendá tu momento de relax.</span>
        </p>
      ...
      {!consentFirmado && !consentNoMigrado ? (
        ...
        >
          ...
          <span>
            <span className="block text-sm font-semibold text-amber-950">Falta firmar tu consentimiento informado</span>
            <span className="mt-0.5 block text-xs text-amber-900/85">
              Es obligatorio antes de reservar. Toca aca para leerlo y firmarlo (1 minuto).
            </span>
          </span>
        </motion.button>
      ) : null}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-serif-premium text-xl font-bold" style={{ color: 'var(--primary-navy)' }}>Mis Citas</h2>
          <motion.button
            type="button"
        ...
        {!activeTreatment && (
          <div className="mb-6">
            <PortalTreatmentEmptyPlaceholder
              ...
              paragraph={
                <>
                  Reservá Body Up, Radiofrecuencia, Crio-lipolisis, Hollywood Peel, Depilacion definitiva, Presoterapia y mas.
                  Elegis dia y horario segun disponibilidad sin superposiciones; al confirmar se guarda en tu cuenta y podes enviar el comprobante por WhatsApp.
                </>
              }
            >
              <motion.button
      ...
      {showDbHero && proximaCita && (
        <ProximaHeroDesdeDb cita={proximaCita} buildWhatsAppHref={buildWhatsAppHref} onAddService={openBooking} />
      )}
      ...
      <AnimatePresence>
        {bookingOpen && uid && (
          <CitasBookingModal
            ...
            onComplete={async (row) => { await onBookingDone(row); }}
          />
        )}
      </AnimatePresence>
...
function ProximaHeroDesdeDb(props: {
  ...
  const estadoLbl =
    ...
    cita.estado === 'pendiente' ? 'Pendiente de confirmacion' :
    cita.estado === 'realizado' ? 'Realizada' : 'Cancelada';
...
function CitasBookingModal(props: {
  ...
  async function confirmar(): Promise<void> {
    ...
    try {
      ...
      if (error || !cita) {
        const msg = error ?? 'No se pudo guardar la solicitud. Intenta nuevamente.';
        setSlotsWarn(msg);
        setSubmitMsg(msg);
        await loadSlots();
        return;
    ...
    >
      ...
      >
        <div className="max-h-[94dvh] overflow-y-auto overscroll-contain p-5 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:p-8">
          ...
          <h2 className="text-serif-premium mb-2 pr-10 text-xl font-bold" style={{ color: 'var(--primary-navy)' }}>Reserva tu turno</h2>
          <p className="mb-6 text-[11px] font-medium uppercase tracking-[0.18em]" style={{ color: 'rgba(0,61,91,0.45)' }}>
            {wizardStep === 1 ? 'Paso 1 — elegi tratamiento' : 'Paso 2 — dia, horario y confirmacion'}
          </p>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {[...CITAS_SERVICIOS_RESERVABLES].map((s) => (
                <button
                  key={s}
                  type="button"
                  onClick={() => { setServicio(s); setHora(null); setWizardStep(1); }}
                  className={`rounded-2xl border px-4 py-3 text-left text-xs font-semibold leading-snug transition ${servicio === s ? 'shadow-md' : ''}`}
                  style={
                    servicio === s
                      ? { borderColor: 'var(--primary-navy)', background: 'var(--primary-navy)', color: '#FFFFFF', boxShadow: '0 8px 24px rgba(0,61,91,0.18)' }
                      : { borderColor: 'rgba(242,215,213,0.65)', background: 'rgba(255,253,251,0.92)', color: 'var(--primary-navy)' }
                  }
                >
                  {s}
                </button>
          ...
          {wizardStep === 2 ? (
            <>
              <div className="my-8 border-t border-[#F2D7D5]/60 pt-8">
                ...
                <div classNme="mt-6 rounded-3xl border p-5" style={{ borderColor: 'rgba(242,215,213,0.5)', background: 'rgba(253,248,245,0.85)', boxShadow: '0 8px 28px rgba(0,61,91,0.04)' }}>
                  ...
                  {availDegraded ? (
                    <p className="mb-3 rounded-xl border border-amber-300/70 bg-amber-50 px-3 py-2 text-[11px] text-amber-950">
                      No pudimos obtener la ocupacion real ({slotsWarn}). Mostramos todas las horas estandar.
                    </p>
                  ) : slotsWarn ? (
                    <p className="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] text-amber-900">{slotsWarn}</p>
                  ) : null}
                ...
                {submitMsg ? (
                  <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-center text-[11px] text-amber-900">{submitMsg}</p>
                ) : null}
              </div>
            </>
          ) : null}
...
function bookingComprobanteWhatsAppHref(servicio: string, fechaMoment: Date, horaSql: string, nombreCliente: string): string {
  ...
  const text = `Hola ${brand.businessName}! Acabo de reservar un turno para: ${servicio} el dia ${fechaHumana} a las ${hora}. Mi nombre es ${nombreCliente}. Nos vemos!`;
  return `https://wa.me/${WHATSAPP_ADMIN_PHONE}?text=${encodeURIComponent(text)}`;
}
